...
class Api::V1::TasksControllerTest < ActionController::TestCase
  ...
  test 'should post create' do
    ...
    assert_equal task_attributes.merge({ author_id: author.id, expired_at: task_attributes[:expired_at].to_date }).stringify_keys, created_task.slice(*task_attributes.keys)
  end
  ...
  test 'should put update' do
    ...
    task.reload
    assert_equal task.slice(*task_attributes.keys), task_attributes.merge({ 'expired_at' => task_attributes["expired_at"].to_date })
  end
